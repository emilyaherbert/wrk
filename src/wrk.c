...
// https://github.com/wg/wrk/pull/258
...
static void usage() {
    printf("Usage: wrk <options> <url>                            \n"
           ...
           "        --supress          Supress prose output       \n"
           "        --per_req          Print per request stats    \n"
...
static int response_complete(http_parser *parser) {
    ...
    // Time that the connect started in microseconds
    // Request latency in microseconds
    u_int64_t diff = now - c->start;
    if(cfg.per_req) {
        printf("%ld,%ld\n", c->start, diff);
    }
    ...
    if (--c->pending == 0) {
        if (!stats_record(statistics.latency, diff)) {
...
static struct option longopts[] = {
    ...
    { "supress",     no_argument,       NULL, 'S' },
    { "per_req",     no_argument,       NULL, 'P' },
